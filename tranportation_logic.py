...
def generate_tus(n_cars):
    ...
    for i in range(n_cars):
        ...
        tu_dict = {
          'name': name, 
          'lat': 53.21720922 - (random.randint(1,10) / 100000), 
          'lon': 6.575406761 - (random.randint(1,10) / 100000),
          'person_capacity': 4, 'battery_state': 0.7, 'battery_capacity': 85 - (random.randint(1,10)),
...
def check_for_new_ride(current_ts, ride_list_test, waiting_list):
    """
    input: current timestamp and list of ride that occur at a timestamp
    ...
    return ride_list_test, waiting_list


objs = generate_tus(14)
waiting_list = []

for i in range(24*60*60):
    current_ts = datum + i
    ride_list_test, waiting_list = check_for_new_ride(current_ts, ride_list_test, waiting_list)

        
    # look for each ride in the waiting list 
    waiting_list2 = waiting_list.copy()
    if len(waiting_list2) > 0:
        for tr in waiting_list2:
            match_tu, distance_tu_tr = find_tu(tr)
            # if a free tu can be found generate a ride and remove it from the waiting list
            if match_tu != -1:
